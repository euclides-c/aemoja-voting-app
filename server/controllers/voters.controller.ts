...
class VotersController {
  ...
  async getVotersById(req: Request, res: Response) {
    const voters = await votersService.readById(req.params.voterID);
    res.status(200).send(voters);
  }

  async getVotersByEmail(req: Request, res: Response) {
    const voters = await votersService.getVoterByEmail(req.params.email);
    res.status(200).send(voters);
  }
  ...
  async removeVoters(req: Request, res: Response) {
    log(await votersService.deleteById(req.params.voterID));
    res.status(204).send();
  }

  async removeVoterByEmail(req: Request, res: Response) {
    log(await votersService.deleteByEmail(req.params.email));
    res.status(204).send();
  }
